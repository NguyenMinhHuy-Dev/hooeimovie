...
export const Cast = () => {
    ...
    useEffect(() => {
        const getCast = async (media_type, id, season_number) => {
            if (season_number === undefined) {
                ...
                await fetch(`https://api.themoviedb.org/3/${media_type}/${id}/season/${season_number}/credits?api_key=${process.env.REACT_APP_API_KEY}`)
                .then(res => res.json())
                .then((data) => {   
                    setCast(data.cast.slice(0, 18));
                    setLoading(false);
